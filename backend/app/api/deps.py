...
def get_current_user(
    ...
    if ecosystem_session:
        try:
            response = httpx.post(
                f"{settings.identity_internal_url}/session-exchange",
                cookies={"ecosystem_session": ecosystem_session},
                timeout=2.5,
            )
            if response.status_code == 200:
                ...
                return UserContext(
                    ...
                    display_name=payload.get("display_name") or payload.get("username") or payload.get("email"),
                    email=payload.get("email"),
                    is_platform_admin=x_platform_admin == "1",
                )
